...
void my_sleep(time_t sleeping_timing) // take argument in microseconds
{
#if 0
    ...
    total_gap_timing = 200 * MILLION; // in micro seconds
...
t_shared *new_shared(time_t time_to_eat, time_t time_to_die, time_t time_to_sleep)
{
    ...
    // init timings
    data->time_to_eat = time_to_eat * THOUSAND;
    data->time_to_die = time_to_die * THOUSAND;
    data->time_to_sleep = time_to_sleep * THOUSAND;
...
void print_state(t_philo *philo, char *str)
{
    ...
    if (starting_time == 0)
        starting_time = current_time.tv_sec * THOUSAND;
    // printf("%d %s", philo->index, str);
    if (ft_strcmp(str, "has taken a fork") == 0)
        printf("%lu: %d %s %d %d\n", current_time.tv_sec * THOUSAND + current_time.tv_usec - starting_time, philo->index, str, philo->index, philo->next->index);
    else
        printf("%lu: %d %s\n", current_time.tv_sec * THOUSAND + current_time.tv_usec - starting_time, philo->index, str);
