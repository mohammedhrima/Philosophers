...
void *action(void *arg)
{
    ...
    if (pthread_mutex_lock(&curr->fork_mutex) == 0 && pthread_mutex_lock(&curr->next->fork_mutex) == 0)
    {
        ...
        if (pthread_mutex_lock(&curr->time_to_live_mutex) == 0)
        {
            ...
            printf("philo %d is eating, time: %d \n", curr->index, curr->time_to_live);
            pthread_mutex_unlock(&curr->time_to_live_mutex);
        }
    ...
    else
    {
        ...
        if (pthread_mutex_lock(&curr->time_to_live_mutex) == 0)
        {
            curr->time_to_live -= TIME_TO_SLEEP;
            if (curr->time_to_live <= 0)
            {
                printf("philo %d died\n", curr->index);
                exit(0);
